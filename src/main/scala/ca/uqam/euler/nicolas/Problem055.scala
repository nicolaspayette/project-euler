...
object Problem055 {
  ...
  def iterations(n: BigInt): Stream[BigInt] =
    BigInt(n.toString.reverse) + n match {
      case x if isPalindrome(x) ⇒ cons(x, Empty)
      case x                    ⇒ cons(x, iterations(x))
    }
  def isLichrel(n: BigInt) = iterations(n).take(50).size == 50
  def main(args: Array[String]): Unit = Answer {
    (1 to 10000).map(BigInt.apply).filter(isLichrel).size
  }
}